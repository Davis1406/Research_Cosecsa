...
@else
    <div class="row">
        ...
        @php
            $role      = $fac->roles->first();
            $isLeadFac = $fac->roles->pluck('title')->contains('Lead Facilitator');
            $photo     = $fac->speaker?->photo?->url ?? null;
            $initial   = strtoupper(substr($fac->name, 0, 1));
            $isSelf    = $fac->id === auth()->id();
        @endphp
        <div class="col-md-6 col-lg-4 mb-3">
            <div class="card shadow-sm h-100" style="border-radius:10px; overflow:hidden; border:1px solid #e9ecef; {{ $isSelf ? 'border-color:#C9A84C;' : '' }}">
                <div class="card-body" style="padding:20px;">
                    <div class="d-flex align-items-center mb-3" style="gap:12px;">
                        ...
                        <div style="min-width:0;">
                            ...
                            <div style="font-size:12px; color:#888; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{{ $fac->email }}</div>
                            <span style="font-size:10px; font-weight:600; text-transform:uppercase; letter-spacing:0.5px; padding:1px 8px; border-radius:10px; {{ $isLeadFac ? 'background:rgba(201,168,76,0.2);color:#9a7d2c;' : 'background:rgba(44,122,75,0.12);color:#2c7a4b;' }}">
                                {{ $role?->title ?? 'Facilitator' }}
                            </span>
                        </div>
                    </div>

                    @if($fac->speaker?->description)
                        <p style="font-size:12px; color:#666; margin-bottom:12px; line-height:1.5;">
                            {{ Str::limit($fac->speaker->description, 80) }}
                        </p>
                    @endif

                    {{-- Social links --}}
                    @if($fac->speaker)
                        <div class="d-flex" style="gap:8px; margin-bottom:14px;">
                            @if($fac->speaker->twitter)
                                <a href="{{ $fac->speaker->twitter }}" target="_blank" style="color:#1da1f2; font-size:14px;"><i class="fab fa-twitter"></i></a>
                            @endif
                            @if($fac->speaker->linkedin)
                                <a href="{{ $fac->speaker->linkedin }}" target="_blank" style="color:#0077b5; font-size:14px;"><i class="fab fa-linkedin"></i></a>
                            @endif
                            @if($fac->speaker->facebook)
                                <a href="{{ $fac->speaker->facebook }}" target="_blank" style="color:#1877f2; font-size:14px;"><i class="fab fa-facebook"></i></a>
                            @endif
                        </div>
                    @endif

                    <div class="d-flex" style="gap:6px;">
                        <a href="{{ route('facilitator.facilitators.edit', $fac->id) }}"
                           class="btn btn-sm flex-grow-1" style="background:#f8f9fa; color:#555; border:1px solid #dee2e6; font-size:12px;">
                            <i class="fas fa-edit mr-1"></i> Edit
                        </a>
                        @if(!$isSelf)
                        <form action="{{ route('facilitator.facilitators.destroy', $fac->id) }}" method="POST"
                              onsubmit="return confirm('Remove {{ addslashes($fac->name) }}\'s account?')">
                            @csrf @method('DELETE')
